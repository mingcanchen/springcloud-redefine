package com.ming.config.config;


import com.ming.config.dto.LoginRequest;
import com.ming.config.dto.Token;
...
public class ConfigClientBootstrapConfiguration {
    ...
    private RestTemplate customRestTemplate(){
        ...
        if (!headers.isEmpty()) {
            template.setInterceptors(
                    Arrays.asList(new GenericRequestHeaderInterceptor(headers)));
        }
    ...
    public static class GenericRequestHeaderInterceptor implements ClientHttpRequestInterceptor{
        ...
        public ClientHttpResponse intercept(HttpRequest httpRequest, byte[] bytes, ClientHttpRequestExecution clientHttpRequestExecution) throws IOException {
            ...
            headers.entrySet().stream().forEach(header -> httpRequest.getHeaders().add(header.getKey(), header.getValue()));
